...
std::vector<Rectangle> segment(int x0, int y0, int x1, int y1, RNG& rng)
{
	...
	return concat({
		segment(x0, y0, x_, y1, rng),
		segment(x_, y0, x1, y1, rng)});
    }
    else if (not splitx and splity) {
	return concat({
		segment(x0, y0, x1, y_, rng),
		segment(x0, y_, x1, y1, rng)});
    } else if (splitx and splity) {
	return concat({
		segment(x0, y0, x_, y_, rng),
		segment(x_, y0, x1, y_, rng),
		segment(x0, y_, x_, y1, rng),
		segment(x_, y_, x1, y1, rng)});
    }
...
void genclusterw(int x, int y, int x0, int y0, int x1, int y1, std::vector<Cell2D>& cells, RNG& rng, double startp=0.5, double decayp=0.9)
{
    ...
    for (Cell2D &c : add)
	genclusterw(c.row, c.col, x0, y0, x1, y1, cells, rng, startp*decayp, decayp);
}
